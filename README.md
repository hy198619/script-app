...
<p align="center">给一人公司主理人和内容创作者的文字临时中转站。</p>

Script App（草稿本）专门用来接住那些还没到“正式归档”这一步的文字：突然冒出的灵感、看视频时随手记下的笔记、需要反复打磨的微信或邮件、临时待办，以及稍后会用到的网址或账号名。

有些内容过两天就没用了；有些要在正式发出前改上几遍；还有一些只是先写下来，稍后再放进笔记软件或知识库。Script 把这些“短保”内容留在一个随手可写的地方。它更像文字的临时中转站，不是另一个等着你整理和维护的知识库。

不值得单独建文件、暂时不知道该存在哪里，甚至最后决定长期留着，也都没问题。
...
- 按轻量常驻的方式设计，可以把窗口收窄，长期放在屏幕一侧；
- 打开窗口即可输入，不需要先创建文件、标签页或标题；
